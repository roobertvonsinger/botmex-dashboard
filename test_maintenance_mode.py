...
def test_maintenance_mode_disabled_by_default(client):
    os.environ["BMX_MAINTENANCE"] = "0"
    res = client.get("/login")
    assert res.status_code == 200

def test_maintenance_mode_redirects_unauth_user(client):
    os.environ["BMX_MAINTENANCE"] = "1"
    res = client.get("/login", follow_redirects=False)
    assert res.status_code == 302
    assert res.headers["location"] == "/maintenance"

def test_maintenance_mode_blocks_api(client):
    os.environ["BMX_MAINTENANCE"] = "1"
    res = client.get("/api/health")
    assert res.status_code == 530
    assert res.json() == {"error": "Sistema en mantenimiento", "maintenance": True}

def test_maintenance_mode_allows_static_assets(client):
    os.environ["BMX_MAINTENANCE"] = "1"
...
def test_maintenance_mode_allows_superadmin(client):
    os.environ["BMX_MAINTENANCE"] = "1"
