...
- **"Sin password" — DESAMBIGUACIÓN (error recurrente, Robert 2026-08-04):** se refiere EXCLUSIVAMENTE a la contraseña de la cuenta de BetMexico — el operador nunca la ve ni la usa, todo corre vía JWT server-side. El login al dashboard/portal (`botmexico.net`, usuario/contraseña PROPIOS del sistema, `POST /api/auth/login`) es SIEMPRE obligatorio para cualquier usuario que no sea SA — no existe ni debe existir un landing sin sesión desde un link de Telegram ni de ningún otro lado. Confundir "sin password de BetMexico" con "sin login al dashboard" ya causó al menos un bug real (copy "Retiro sin password" expuesto textual al operador) y una descripción de flujo incorrecta en la misma sesión que lo corrigió. Todo el sistema existe precisamente para que el operador NO PUEDA bypassear el dashboard e irse directo a BetMexico — el login es el mecanismo de ese candado, no un obstáculo a saltarse.
- **Portal del operador (`/user/{id}`) — corrección de campo pendiente de implementar (Robert 2026-08-04):** debe ser UNA SOLA vista, sin separar "misión activa" de "grid de cuentas" y sin exponer IDs de misión, tags, ni conteos de fallidos — eso es ruido que no ayuda al operador ni a nadie. Regla de visibilidad real: el operador **solo** ve cuentas con un depósito ya aterrizado o actualmente en proceso. Si un intento de depósito falla, esa cuenta desaparece por completo de su vista y de su acceso — no queda como card "fallida", no se le muestra un stat de fallos. La implementación actual (2026-08-04) todavía tiene dos vistas separadas con `mission_id` visible y un resumen con conteo de fallidos — **no refleja esta regla todavía**, queda pendiente de rediseño.
