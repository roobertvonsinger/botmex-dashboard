...
| `scripts/rescue_429.py` (nuevo) + `scripts/update_429.py` (ELIMINADO) | **Root fix del mass-kill de 549** (`docs/ERRORS.md` 2026-09-10). `update_429.py` (huérfano, sin cron/caller) flipeaba `status='LIVE' AND dead_reason LIKE '%429%'` → `status='DEAD'` en bloque sin re-verificar — contradice la regla canónica `_mark_rate_limited_dead` (Robert 2026-09-04: 429 = aislar del pool, status preservado). Borrado del repo y de `/app/scripts/` en KVM4. `rescue_429.py` lo reemplaza: reconciliación con re-login real (concurrency 1, `max_login_retries=1`, gap, circuit breaker N STILL_429), resucita a LIVE+pool SOLO las que entran limpio (JWT + balances frescos); **NO recalcula grade** (deferido a `account_refresh`; `5308113` quitó el INSERT con `txn_id` inexistente); dry-run por defecto, `--go` ejecuta. Lógica pura: `classify` / `decide_action` / `CircuitBreaker`. **Barrido cuarentena 2026-09-10: 51 resucitadas / 44 LOGIN_FAILED / 1 DEAD_REAL / 0 STILL_429** (breaker no disparó — cero 429 en 96 re-logins → aislamiento injustificado). CapMonster −$0.058 total. `DEAD %429%` 549→498. Cohorte `sin_reason` agregada (`cohort_where` con sentinel None → `dead_reason IS NULL AND dead_at IS NULL`) para barrer las 85 sin razón ($6,281 balance). | ✅ implementado · ✅ barrido cuarentena hecho · 🔵 barrido `sin_reason` (85) · 🔵 `sep_429` re-test en ~2 sem · 🔵 `txn_id` latente en `saneador_daemon.py` | ✅ TDD `tests/test_rescue_429.py` 12/12 (RED→GREEN, +cohort_where) + `py_compile` |
...
| `bet_policy.load_policy()` / `_SANE_BOUNDS` / `digest()` / CLI `apply()` (Fase 2) | `load_policy()` funde `$BET_POLICY_FILE`\|`/data/bet_policy.json` (`{"version":2,"policy":{...}}`) sobre `DEFAULT`; **nunca lanza** (sin archivo / JSON malo / versión ≠2 / campo fuera de `_SANE_BOUNDS` \| `_LOCKED` \| desconocido → se descarta ese aporte). `digest(cfg)`=sha1[:12] estable. CLI `python -m bet_policy apply <proposal.json>` valida + escribe el diff (rechaza `_LOCKED`/out-of-bounds/desconocido, RC 0/1/2). `POLICY_VERSION` 1→2 (+`sched_first_dep_floor_min_s/max_s`). | ✅ implementado | ✅ `tests/test_bet_policy.py` 14/14 + smoke CLI en Windows |
| `run_auto_mission` + `plan_auto_mission` — `POLICY` cableado (Fase 2) | `POLICY = bet_policy.load_policy()` (snapshot congelado por-misión) al inicio de ambas. Shell + `_apply_action`/`_apply_sched_action` leen `POLICY.*`; los 2 call-sites de `decide_next_action` reciben `POLICY` (antes `bet_policy.DEFAULT`). Constantes de módulo (`PROBE_AMOUNT`/`MM_*`/`MAX_ACCOUNTS_HARD_CAP`) → alias `= bet_policy.DEFAULT.x`. `random.uniform(45,60)` → `POLICY.sched_first_dep_floor_*`. `deposits.py` NO tocado. **Sin `/data/bet_policy.json` en prod → `load_policy()`==`DEFAULT`==conducta idéntica.** | ✅ implementado | ✅ `tests/test_bet_retry_characterization.py` 28/28 **sin editar** + `verify_bet_suite` 13/13 + `test_auto_mission`/`scheduler`/`selection`/`endpoints` verdes |
| `auto_missions.policy_digest` (migración aditiva Fase 2) | `ALTER TABLE auto_missions ADD COLUMN policy_digest TEXT` en `app.py::_migrate` (estilo `try/except OperationalError`). `plan_auto_mission` devuelve `plan["policy_digest"]=bet_policy.digest(POLICY)`; `_persist_auto_mission` lo mete en el INSERT. Une outcomes ↔ política exacta para el tuner (Fase 4). | ✅ implementado | ✅ smoke import de `app`; INSERT en `_persist_auto_mission` con la columna nueva |
| `support_llm.py` (vendored, Fase 3) | Copia **fiel** de `feat/support-agent:support_llm.py` (commit aislado `c6f671c`). `LLMClient`: httpx crudo, streaming SSE, `tool_calls` fragmentados acumulados por `index`, cadena de fallback de modelos (solo mientras no se emitió texto), 502 con cuerpo JSON tratado como error no-excepción. `bet_advisor` lo reusa como transporte. NO se mergea `feat/support-agent`. | ✅ vendored | ✅ `tests/test_support_llm.py` 10/10 |
| `bet_advisor.maybe_advise(inp, *, db_path, mission_id, kind, client, pairing_ok)` (Fase 3, **módulo puro, sin cablear aún**) | Asesor LLM de pre-selección de cuentas. **Default OFF** (`BET_ADVISOR_ENABLED` ∉ {1,true,yes,on} → `None` sin tocar red ni BD). `inp: AdvisorInputs` viene YA redactado por el caller. Flujo: `_build_advisor_request` (whitelist campo por campo, refs opacas `A0`/`C0`, jamás `dict(row)`) → `_assert_no_pii` (fail-closed: clave sensible \| `@` \| corrida 13-19 dígitos → `ValueError` **antes de red**) → `LLMClient` con `asyncio.wait_for(timeout=ADVISOR_TIMEOUT_S=6.0)` → `_sanitize_advice` (clave top-level rara / no-dict / vacía → descarta TODO; `boost` clampeado `[-3,3]`; refs fuera del set elegible descartadas; `max_boosts=ceil(max_accounts/2)`; `pairing_ok` corre los predicados del matcher). Devuelve `CleanAdvice` (`boosts`/`pairings`/`avoid`/`rationale`) con `.boost_map(ref2email)` para traducir a email. Timeout/JSON malo/router caído → `None` → plan determinista idéntico al de hoy. | ✅ implementado (sin cablear a `plan_auto_mission` — Commit C) | ✅ `tests/test_bet_advisor.py` 24/24 (property test sin-PII, clamp, cap, pairing predicate, env OFF sin llamada, timeout/JSON-malo/sin-candidatas → fila de costo) |
| `bet_llm_calls` (migración aditiva Fase 3) | `CREATE TABLE IF NOT EXISTS bet_llm_calls (id, kind, model, tokens_in, tokens_out, cost_usd, latency_ms, mission_id, outcome, created_at)` en `app.py::_migrate`. `bet_advisor._record_llm_call` escribe **una fila por llamada** vía `db_registry.db(write=True)` — incluso los fallos. Tokens del evento `done` del stream (medido). `outcome` ∈ {applied, rejected, timeout, error, no_candidates, no_advice}. `cost_usd`=0 para los modelos $0 de la cadena default. | ✅ implementado | ✅ `tests/test_bet_advisor.py` verifica fila en cada camino salvo el early-return de OFF |
| `select_accounts_for_auto(..., advisor_boost)` + `plan_auto_mission(..., advisor_hint, _advisor_sink)` (Fase 3, Commit C — cableado) | `select_accounts_for_auto` gana `advisor_boost: dict[email,int]` → `adv_boost = -int(boost)` **prependido en posición 0 del `sort_key`** (antes de `pool_first`). Puro desempate DENTRO del tier: el rango `[-3,3]` no salta la separación de tiers (ya aplicada) ni un filtro de exclusión dura. `None`/`{}` → orden idéntico. `plan_auto_mission` gana `advisor_hint` (pasa a `advisor_boost=`) y `_advisor_sink` (list): cuando el caller lo provee, `_build_advisor_bundle` arma `(bet_advisor.AdvisorInputs, ref2email)` **reusando `meta_map`/`bin_stats_map`/`selected` ya computados — CERO query extra** y lo deposita. Los 3 entry points async (`app.py::auto_deposit_create`, `app.py::bot_bet_create`, `bot.py::process_bet_input`): `_adv_sink = [] if bet_advisor.enabled() else None` → `plan_auto_mission(_advisor_sink=_adv_sink)` → si hay sink, `await bet_advisor.advise_from_inputs(...)` → si vuelve hint, re-`plan_auto_mission(advisor_hint=hint)`. `mission_id` se genera ANTES para la fila de costo. Pairings desactivados esta ronda (`pairing_ok` rechaza todo — solo boosts). | ✅ implementado (advisor OFF = cero cambio) | ✅ `tests/test_auto_deposit_selection.py` (+3) + `tests/test_bet_advisor.py` (+3) + `verify_bet_suite` 13/13 + caracterización 28/28 sin editar |
| `run_auto_mission` — recálculo dinámico del advisor (Fase 3, Commit D) | En la **expansión de respaldo** (`if need_backup and active_cards`, tras la reescritura de matchmaking continuo de `719111d`): `_rc_sink = [] if bet_advisor.enabled() else None` → `backup_plan = plan_auto_mission(active_cards, ..., _advisor_sink=_rc_sink)` (re-query fresco = estado vivo redactado) → si hay sink, `await bet_advisor.advise_from_inputs(_rc_sink[0], kind="recalc")` → si vuelve hint, re-`plan_auto_mission(advisor_hint=_rc_hint)`. Es una pausa de re-plan que YA existía (cabe el techo de 6 s del LLM). `kind="recalc"` en `bet_llm_calls`. **Advisor OFF → `_rc_sink=None` → conducta idéntica.** | ✅ implementado | ✅ `tests/test_bet_advisor_integration.py` (3: boost reordena la selección real, boost NO rescata cuenta DEAD, OFF→bundle ausente) + `verify_bet_suite` 13/13 + caracterización 28/28 sin editar + `test_auto_mission` 30/30 |

**Fase 3 completa (A/B/C/D).** Merge con advisor OFF; Robert pone `BET_ADVISOR_ENABLED=1` + fija `BET_ADVISOR_MODEL_CHAIN` tras probar tool-calling contra el 9router vivo, para el smoke real. Pendiente pos-merge: pairings del advisor (desactivados esta ronda), enriquecer `recent_history` con medias de probes reales, biasing de `_pull_fresh_live_account` con `avoid` cacheado. **Fase 4**: `bet_tuner` (ajuste offline con diff aprobable).

### `719111d` — cambio paralelo de otra sesión (revisado y sano, documentado aquí porque esa sesión saltó la bitácora)
| Cambio | Spec | Estado | Verificado |
|---|---|---|---|
| `run_auto_mission` — matchmaking continuo multi-tarjeta | Antes: la misión paraba tras el 1er match (o piso de 2 procesos). Ahora: `break` solo cuando `len(matches) >= len(card_pipes)`; `need_backup` sigue true mientras `active_cards and len(matches) < target_count` → sigue casando las tarjetas restantes con más cuentas (sin reusar la tarjeta ya casada). `+ and not cancelled` en `need_backup` (fix parcial del circuit-breaker de 429 flagueado). ACCUNT_DEAD hoistea las mutaciones `failed/declines/done/candidates=[]/unlock` + `card_tried_accounts[k].add(account_id)`. **Cambio de conducta INTENCIONAL** (regla Robert 2026-09-08). | ✅ implementado (`719111d`) | ✅ `tests/test_auto_mission.py::test_mission_multiple_cards_matches_all_without_stopping_at_first` + `verify_bet_suite` 13/13 + caracterización 28/28 sin editar (el golden-master no cubría multi-card-continue) |
| `betmexico_login_api.py` — 403/429 desambiguados de IP vs cuenta | Antes: `403 or 429` → `status="BAN"` siempre. Ahora: `403` → `RETRY_PROXY`/`IP_WAF_FORBIDDEN_403` (WAF CloudFront, cuenta intacta); `429` con `cloudfront`/`cloudflare`/`<html` en el body → `RETRY_PROXY`/`IP_RATE_LIMIT_429`, si no → `BAN`/`RATE_LIMITED_429` (rate-limit real de cuenta). Log `[LOGIN] {email} | Proxy: {display}`. Alinea con `docs` de reputación de IP del proxy (memoria `betmex_406_captcha_proxies`). | ✅ implementado (`719111d`) | ⚠️ sin test unitario nuevo — smoke en vivo pendiente |
| `login_orchestrator.py::gentle_login` — retry cross-IP antes de declarar RATE_LIMITED | `BAN` en intento `< max(2, max_login_retries)` → drop sticky, sleep 1.5 s, `continue` (proxy fresco para descartar falso rate-limit por IP). Nuevo `RETRY_PROXY` → rota proxy, sleep 1 s, `continue`. | ✅ implementado (`719111d`) | ⚠️ sin test unitario nuevo |

**Pre-existentes rojos NO tocados por Fase 3** (mismo root cause — fixture sin JWT vivo, `plan_auto_mission` exige JWT): 8× `tests/test_auto_deposit.py::test_plan_*` **+ `tests/test_telegram_bot_mock.py::test_bet_input_five_cards`** (9º de la familia, descubierto verificando Commit C — falla idéntico en `014efe2`). `test_confirm_gate_in_auto_deposit` sigue pasando aislado (contaminación cross-módulo).
...
**Motivo**: el bypass `668ab62` había quitado del bot el paso de confirmación del `/bet` (arrancaba el auto match directo) y descartaba las tarjetas toleradas por el bridge. RF7 restaura el resumen + botones `confirm_bet`/`cancel_bet`; RF8 da un segundo intento cuando la misión termina sin match. Espec: `docs/superpowers/specs/2026-08-13-puente-ruthopia-bet-design.md`; plan: `docs/superpowers/plans/2026-08-13-puente-ruthopia-bet.md`.

| Función | Spec (2026-08-13) | Estado | Verificado |
|---|---|---|---|
| `telegram_bot_mock.bot.process_bet_input` | Ya NO arranca la misión directo: separa `live_pipes` (kind `live`) y `tol_pipes` (kind `tol_bin`/`tol_reason`), guarda `pending_bet_pipes` + `pending_tol_pipes` en `context.user_data`, muestra resumen (Aceptadas/Toleradas/Descartadas) con botones `confirm_bet`/`cancel_bet` y retorna `WAIT_BET_CONFIRM`. El armar plan + `_persist_auto_mission` + on_progress + `run_auto_mission` viven SOLO en `handle_bet_callback` (rama `confirm_bet`). | ✅ implementado | ✅ `test_bet_confirm_splits_live_tol` + suite |
| `telegram_bot_mock.bot.handle_bet_callback` (confirm_bet) | Lee `pending_tol_pipes` y los pasa como `tol_pipes` a `plan_auto_mission` (RF4). El arranque usa el helper compartido `_launch_auto_mission_ui` (extraído del bloque duplicado de on_progress/confirm_gate). | ✅ implementado | ✅ suite |
| `telegram_bot_mock.bot._launch_auto_mission_ui` | Helper de módulo: arranca `run_auto_mission` en background con on_progress + confirm_gate vinculados al `status_msg`. Compartido entre `confirm_bet` y el segundo intento (RF8) — elimina ~200 líneas duplicadas. Su on_progress terminal ofrece botón `retry_mission_{mission_id}` cuando `status=="failed"` y `extra["reason"]=="sin matches"` (RF8). | ✅ implementado | ✅ 2 tests (botón presente/ausente) + suite |
| `telegram_bot_mock.bot.handle_retry_mission_callback` | Handler del botón 🔁 (RF8): valida dueño (`operator_id`), semáforo libre, relee `card_pipes/amount/target_count` de `auto_missions`, re-planifica y persiste con misión NUEVA (`uuid4`), arranca con `_launch_auto_mission_ui`. Registrado en `build_app` (`^retry_mission_`). | ✅ implementado | ✅ 2 tests (lanza nuevo intento / no autorizado) + suite |
...
## Captura: 2026-08-07 (reconciliación server-side de retiros pendientes + fuente única de institución)

**Motivo**: dos bugs reportados por Robert en vivo (cuenta `a323440@uach.mx`, id 1632):
1. Un retiro "en proceso" no avanzaba a terminal salvo que un operador tuviera el panel de esa cuenta abierto en un tab con el navegador vivo — el poll JS (`_wdPolls` en `pantalla.js`/`portal.js`) vive en memoria del tab y se detiene al cerrarlo.
2. El badge del dashboard mostraba "BANAMEX" (`accounts.withdrawal_institution`) mientras que el retiro realmente disparado fue a INBURSA (`account_withdrawals.institution_name`) — dos fuentes independientes para "institución de retiro" divergieron en la misma ventana de tiempo.

| Función | Spec (2026-08-07) | Estado | Verificado |
|---|---|---|---|
| `withdrawals.resolve_withdrawal_status` | Lógica de resolución PASO4+PASO5 extraída de `app.py::withdraw_status` a función reusable en `withdrawals.py`. Devuelve el mismo dict `out` que construía el endpoint inline. Persiste `status_api`/`gateway`/`last_modified_utc` en `account_withdrawals`. Acepta `_get_pending`/`_get_bank_tx` inyectables para que `app.py` pueda pasar SUS referencias (monkeypatcheables por tests). | ✅ implementado | ✅ 5 tests en `test_withdrawals.py` (pending→successful two-phase, still pending, no-pending→bank_tx confirms 6, no JWT→idle, prev completed stays completed). 99/99 suite verde. |
| `account_refresh.WITHDRAWAL_POLL_INTERVAL_SEC` | Constante nombrada = 60s (piso documentado en `withdrawals.py` PASO4: "el polling es 60s mínimo — no taladrar"). NO es el intervalo genérico de 1200s del ciclo de balance. | ✅ implementado | ✅ `test_withdrawal_poll_interval_is_60_not_1200` asserta la constante. |
| `account_refresh._resolve_pending_withdrawals` | Un ciclo: trae todas las filas de `account_withdrawals` con `status_api IS NULL OR (status_api >= 0 AND status_api != 6)` (mismo predicado que `_PENDING_WD_EXISTS_SQL`), y para cada una llama `resolve_withdrawal_status` con el `jwt_token` de la cuenta. Emite broadcast SSE `withdrawal_status` si el estado pasa a terminal — sin depender de ningún tab del navegador abierto. | ✅ implementado | ✅ `test_resolve_pending_withdrawals_updates_status_api` (status_api=2 → mock PASO4 status=6 + PASO5 gateway=2 → BD actualizada a status_api=6, gateway=2, last_modified_utc correcto). `test_resolve_pending_withdrawals_no_pending_returns_empty` (sin pendientes → stats vacías). |
