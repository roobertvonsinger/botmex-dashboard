...
|---|---|---|---|
| **`jwt_keeper.select_refresh_candidates`** (lógica pura) | Filtra cuentas a re-loguear: LIVE, grade en `JWT_KEEPER_GRADES` (A+/A/B), publicada, NO en cooldown, NO lockeada, JWT ya expirado o expira en <`REFRESH_AHEAD` (24h). Ordena por grado (mejor primero) + urgencia (menor exp), corta en `BATCH` (20, subido de 12 el 2026-07-11 para drenar backlog). NO selecciona JWT con margen (sigue sirviendo). | ✅ implementado | ✅ 13 tests unitarios `test_jwt_keeper.py` verdes |
