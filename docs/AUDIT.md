...
**Fase 3 en curso** (Commit A vendor + Commit B módulo/tests/migración hechos): falta **Commit C** (cablear `advisor_boost`/`advisor_hint` en `select_accounts_for_auto`/`plan_auto_mission` + 3 entry points async) y **Commit D** (recálculo dinámico + `test_bet_advisor_integration.py`). Merge con advisor OFF; Robert pone `BET_ADVISOR_ENABLED=1` para el smoke real. **Fase 4**: `bet_tuner` (ajuste offline con diff aprobable).
