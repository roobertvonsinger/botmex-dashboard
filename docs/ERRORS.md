...
- **Verificado**: 12 tests unitarios (`test_grading_a_plus_m7.py`) cubren M7 (masacre reciente→C, descansada→C, 5 aislados→C, pocos→B, reciente→D, sin fails→A) + ciclo A+ (1 decline sigue A+, 2 seguidas→B, aprobado resetea, decline-aprobado-decline NO baja, ruido no-banco no toca, no-A+ es no-op). **Pendiente**: validar end-to-end en prod tras deploy (un 3DS real seguido de 2 declines de banco).
