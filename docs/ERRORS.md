...
- **Bug 1 — circuit breaker 429 ciego (real, hot path)**:
  - **Síntoma**: al pegar 2 cuentas consecutivas con `RATE_LIMITED` (429) en FASE 1 matchmaking, el log mostraba `🛑 [CIRCUIT BREAKER] ... abortando misión` pero la misión **seguía** procesando el resto del plan (acc3, acc4, ...), re-disparando el breaker en cada una y marcándolas todas `RATE_LIMITED_DEAD`. El propósito del breaker ("evitar quema de captchas bajo rate-limit") quedaba anulado.
  - **Causa raíz**: el loop dispatcher de FASE 1 (`auto_deposit.py:2095`) era `while not _cancelled():`. `_cancelled()` lee `_m_status(mission_id) == "cancelled"` — **status en BD**. El breaker (`_apply_action`, rama `AK.ACCOUNT_DEAD` + `circuit_breaker_hit`, L1927-1939) setea la variable **local** del closure `cancelled = True`, hace `_broadcast_mission(..., "aborted")` y `return "break"` — pero **nadie escribe status terminal a BD**. El `while` externo re-evaluaba solo `_cancelled()` → seguía `False` → siguiente cuenta. El guard `need_backup` (L2136) sí chequeaba `and not cancelled`, así que solo la expansión de respaldo se saltaba; el plan principal seguía moliendo.
  - **Fix** (`fix(bet): circuit breaker de 429 aborta la mision de verdad`): `auto_deposit.py:2095` → `while not _cancelled() and not cancelled:` (la var ya se leía como free-var del closure en L2136). `auto_deposit.py:2465` → añadido `and not cancelled` para saltar también el `cross_account_gap_s` de respiro tras el aborto. El broadcast "aborted" y el status de cierre `cancelled` no cambian.
  - **Verificación**: `tests/test_bet_retry_characterization.py::test_char_rate_limit_circuit_breaker_current_behavior` documentaba explícitamente este "posible bug" (docstring viejo L200-209) → reescrito a la conducta correcta (`test_char_rate_limit_circuit_breaker_aborts_mission`: `H.dead == ["acc1@x.com", "acc2@x.com"]`, no las 4). RED confirmado (código viejo tocaba las 4) → GREEN. `verify_bet_suite` 13/13, caracterización 20/20, `test_bet_retry_policy` 66.

- **Bug 2 — 8 `tests/test_auto_deposit.py::test_plan_*` rojos (solo fixture, cero prod)**:
  - **Síntoma**: `test_plan_never_uses_married_card`, `test_plan_assigns_pool_cards`, `test_plan_normalizes_4part_pool_cards`, `test_plan_feasibility_check`, `test_plan_estimates_total`, `test_plan_excludes_accounts_with_recent_declines`, `test_plan_max_accounts_scales_with_card_count`, `test_plan_max_accounts_hard_cap_at_10` → `StopIteration` / `assert 0 == N` / `assert False` (planes vacíos).
  - **Causa raíz**: el Blindaje Canónico 2026-09-04 metió en `plan_auto_mission` (`auto_deposit.py:710-714`, y también en el fallback de rotación L758-766) el filtro `AND jwt_token IS NOT NULL AND length(jwt_token) > 20 AND jwt_expires_at > (strftime('%s','now') + 120)`. El helper `_add_account` de `test_auto_deposit.py` insertaba `jwt_expires_at` pero **no `jwt_token`** (→ `NULL`), y la cuenta base `b@test.com` del `conftest.py::seed_db` tampoco lo tenía → toda cuenta de test se filtraba.
  - **Fix** (`test(bet): sembrar jwt_token en fixtures de test_plan_*`): `_add_account` inserta `jwt_token="jwt_live_placeholder_0123456789"` (30 chars > 20, mismo patrón que `test_bet_advisor_integration.py`). `conftest.py::seed_db` da JWT vivo a `b@test.com` (la cuenta LIVE sana — a@ lockeada, c@ DEAD). **`plan_auto_mission` NO se toca**: el gate JWT es conducta de producción correcta y deseada (Robert 2026-09-04).
  - **Verificación**: `tests/test_auto_deposit.py` 22/22. Barrido de regresión (7 archivos que fallaban en full-run, aislados, con y sin el cambio): **15 fallos idénticos** con/sin — todos pre-existentes (contaminación cross-módulo `BMX_MAINTENANCE` + mocks stale post-Fase-3 en `test_bot_bet.py`/`test_telegram_bot_mock.py`/`test_auto_missions_migrate.py`). Cero regresión.

- **Lección**: un flag de aborto que vive solo en el closure (no en la fuente de verdad que consulta el loop) no aborta nada — el `break` de la iteración actual no es un `break` del `while`. Si `_cancelled()` es la condición del loop y lee BD, todo camino de aborto tiene que **o** escribir a BD **o** ser añadido a la condición del `while`.
...
- **Causa raíz (verificada, no estimada)**: `renapo_validator.py` golpeaba `_RENAPO_CURP_API = "https://valida-curp.com/api/curp"`. **`valida-curp.com` no existe (NXDOMAIN — `nslookup` falla, curl directo/vía DI = 000)**. Cada GET de detalles de una cuenta sin CURP dispara `_async_val_renapo` (`app.py:3250`) → 32 candidatos × `call_with_proxy_failover(captcha_retries=3)` → como el host no resuelve, cada intento lanza `ConnectError` y el failover cicla **el pool completo (~500 proxies)** → quemazón masiva `NO_HOST_CONNECTION` contra un host muerto (785/785 en CSV de DI; ~68k + ~41k líneas en `.3`/`.2`). El diagnóstico que culpó a DataImpulse fue falso: DI es sano para BetMexico (**781/781 success** en el CSV `usage_2026-08-12_2026-08-12.csv`, y `curl` en vivo vía `gw.dataimpulse.com:823` → 401/405 a `paymentsapi.betmexico.mx`, no 502).
- **Fix**:
  1. `renapo_validator.py` — pre-check DNS (`socket.getaddrinfo`) como gate: si el endpoint no resuelve → fallback local inmediato, **cero gasto de pool**. Endpoint apuntado al oficial `consultas.curp.gob.mx/CurpSP/curp2.do` (host vivo, 403 a GET directo → fallback sin reintentos). Intentos acotados (`max_attempts=2`, `captcha_retries=1`) y solo `candidates[:3]`. Cache por (fullname, birthdate, address).
  2. `proxy_pool.py` — nuevo parámetro `max_attempts` en `call_with_proxy_failover` (tope de gasto del pool, default sin cambio). KVM4 quedó alineado al estado correcto del repo: **solo DataImpulse activo** (823 + 10000-10499), `_EXCLUDED_PROXY_HOSTS = ("litport", "proxy001", "nodemaven")` (nodemaven se agrega porque el `betmexico_config.ADMIN_PROXIES` del bot en KVM4 aún lo lista — la exclusión aplica al pool combinado).
  3. `test_renapo_validator.py` — mock del gate DNS en los tests de failover (determinismo offline).
- **Verificación**: suite local 438/438 + 2 fails pre-existentes de withdrawals (ajenos). Deploy a KVM4 (`scp` + `docker restart betmexico-web`), MD5 de ambos archivos = repo. Pool desplegado: 501 proxies, **todos `gw.dataimpulse.com`**. Failover real end-to-end dentro del container → `405` vía `gw.dataimpulse.com:10018/10174` (llega a BetMexico, sin 502). Logs limpios post-restart (0 `NO_HOST_CONNECTION`).
- **Lección**: nunca atribuir `502 NO_HOST_CONNECTION` de un proveedor a su pool sin verificar **qué host destino** lo generaba — un dominio NXDOMAIN golpeado con failover quema el plan de TODOS los proxies del pool y produce el patrón idéntico al de un proveedor caído.

## Guardarrail de retiro (gateway/dígitos mismatch) ciego en silencio — PASO5 pegaba al endpoint equivocado (corregido 2026-08-08)

- **Síntoma**: `withdrawals.py::get_bank_transaction` (PASO5, "auditar el rail externo" de un retiro) nunca disparaba las alertas `gateway_mismatch` (retiro fue a TARJETA en vez de SPEI, bug#3) ni `digits_mismatch` (retiro aterrizó en una cuenta con dígitos distintos a los esperados, bug#1) — `static/pantalla.js::_withdrawStatusHtml` nunca mostraba `⚠️ BetMexico mandó el retiro a TARJETA` ni `⚠️ El retiro fue a dígitos distintos`, sin importar si hubo mismatch real.
- **Causa raíz**: `get_bank_transaction` golpeaba `GET /api/wallet/bankTransaction/{tx_id}` esperando leer `gateway`/`gatewayType` y `lastAccountDigits`/`account` de la respuesta. Verificado en vivo (2026-08-08, cuenta real 1632, tx `bb4a346c-e97a-463f-8167-0649c61a6656`, retiro SPEI exitoso de $100): la respuesta REAL de ese endpoint para retiros es `{"id","transactionStatus","amount","transactionTypeDescription","reference","transactionStatusDescription","lastModifiedUtc"}` — **nunca** trae `gateway` ni `lastAccountDigits`/`account`. Por eso ambos flags quedaban `False` siempre: no porque nunca hubo mismatch, sino porque la fuente nunca traía los campos que el código intentaba leer. El endpoint correcto SÍ existe y SÍ trae esos campos — verificado en vivo, misma tx, contra `GET /api/Wallet/Transactions/ByUser`: item con `id==tx_id` trae `gateway:2, account:"5646", lastAccountDigits:"5646"` (ya documentado en `docs/RECON_BETMEX_API.md:119-134`, nunca se había conectado al guardarrail real).
- **Fix**: `withdrawals.py::get_bank_transaction` reimplementado para golpear `GET /api/Wallet/Transactions/ByUser?pageNumber=1&pageSize=50` y buscar el item con `id==tx_id` (paginado, shape `{"data":{"results":[...]}}`), extrayendo de ahí `gateway`/`lastAccountDigits`/`account`/`status`/`date`. Mismo nombre de función y mismo contrato de salida (`gateway_spei`, `gateway_mismatch`, `digits_mismatch`, `expected_digits`, `actual_digits`, `transactionStatus`, `lastModifiedUtc`) — cero cambios en `app.py` (línea 3819, `_get_bank_tx=get_bank_transaction`) ni en `account_refresh.py` (usa el default del módulo). Si `tx_id` no aparece en la primera página del historial, ahora lanza `RuntimeError` explícito en vez de devolver flags en `False` sin evidencia.
- **Verificación**: TDD completo, RED confirmado (6 tests fallando contra la implementación vieja) antes del fix. `pytest test_withdrawals.py test_withdrawals_endpoints.py test_withdrawals_migrate.py test_account_refresh.py` → 104/104 verdes tras el fix.
- 🔵 Pendiente: no hay evidencia empírica de un mismatch REAL disparando la alerta en producción (el caso probado en vivo fue un retiro SPEI correcto, sin mismatch). Falta confirmar con un caso real de tarjeta/dígitos distintos, o al menos un smoke con datos mock inyectados server-side, antes de confiar el guardarrail al 100%.

## AttributeError: 'LoginResult' object has no attribute 'status' (corregido y deployado 2026-08-08)
...
- **Fix**: `bot.py:707-708` — `elif login_res.status == "DEAD"` → `elif login_res.account_dead:`, y `login_res.error_message` → `login_res.error`. Commit `a9d58fb`.
- **Verificación**: nuevo test `tests/test_telegram_bot_mock.py::test_run_check_task_marks_dead_account_without_crashing` (antes no existía NINGÚN test que ejercitara `_run_check_task`, por eso el bug pasó el commit anterior). `pytest tests/test_telegram_bot_mock.py` → 23/23. **Deploy**: `scp` + `docker restart betmexico-mock-bot` en KVM4, 2026-08-08 12:50:15 (hora local). Confirmado con `docker logs betmexico-mock-bot` completo: última línea del log ANTES del restart con este error es `10:37:57`; **cero ocurrencias nuevas después de `12:50:15`** — el fix quedó sostenido en producción.
- **Trampa de log viejo — mecanismo REAL (corregido 2026-08-08, la explicación anterior de "buffer de Docker" estaba mal)**: el visor de logs del dashboard (`/api/logs`, `/api/logs/telegram`) NO lee `docker logs` — lee archivos persistentes en `/data/logs/` (`dashboard.log`, `telegram_bot.log`, `telegram_mock_bot.log`, ver `app.py:1824-1891` `_tail_log_file`). Ese `/data` es un volumen montado (`docker-compose.yml`), así que **sobrevive un `docker restart` intacto** — no se trunca, no se rota por tiempo. Si el bot tiene poco tráfico, un traceback viejo puede quedarse dentro de la ventana de "últimas 300 líneas" durante HORAS o DÍAS después de que el fix ya está deployado y funcionando. Confirmado en campo: el mismo traceback de `LoginResult.status` (ya arreglado y deployado a las 12:50) seguía apareciendo en el visor casi 3 horas después porque el archivo simplemente no había crecido lo suficiente para empujarlo fuera de la ventana — y aparecía DOS VECES, porque el logger raíz también lo escribía en `dashboard.log` además de `telegram_mock_bot.log`. Se limpiaron manualmente los bloques muertos de ambos archivos (2026-08-08) para dejar de confundir. Antes de asumir que un fix deployado no sirvió: (1) comparar el timestamp exacto de la línea de error, no solo su posición visual; (2) si es viejo pero sigue estorbando, se puede limpiar directo del archivo en `/data/logs/` sin riesgo (es solo bitácora, no dato operativo).
