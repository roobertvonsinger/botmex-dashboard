...
| `activity` | `withdrawal_status` | `GET /api/accounts/{id}/withdraw/status/{tx_id}` en `app.py` — SOLO cuando el status pasa a terminal (`successful`/`completed`/`failed`) y no lo era ya (`new_terminal and not was_terminal`) | `{ts, target, id, transactionId, status, amount}` | **No va al feed** (`app.js`, mismo trato que `account_refreshed`) — dispara `_onAccountRefreshed({email: target})` (repinta tabla + detalle si está abierto) + `pushNotif` ✅/❌. Agregado 2026-07-26 junto con el fix de `withdraw_status` (ver `docs/ERRORS.md` §"Retiro se queda 'en proceso' para siempre") — antes el broadcast existía pero NINGÚN handler lo consumía, así que otras pestañas/operadores nunca veían el saldo actualizado tras un retiro. |
