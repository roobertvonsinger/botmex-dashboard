...
| `activity` | `auto_mission` | `run_auto_mission()` en `auto_deposit.py` vía `_broadcast_mission()` — Modo Auto V2 (2026-07-28). Un evento por transición de fase | `{ts, mission_id, status, who, who_color, who_id, ...extra}`. `status` ∈ `started` (emitido por `POST /api/deposits/auto` al lanzar, con `accounts`), `matching` (fase 1, con `accounts`), `match` (par casado, con `email`/`card_tail`), `scheduling` (fase 2; progreso con `email`/`completed`/`total` o aborto con `aborted:code`), terminal ∈ `completed`/`failed`/`cancelled` (con `deposited`/`approved`/`failed`/`accounts` o `reason`). El endpoint cancel también emite `status:'cancelled'` directo desde `app.py` | `pushActivityEvent()` · app.js `connectSSE` (pushNotif en hitos terminales: 🤖 completada / ❌ falló / 🛑 detenida) · drawer Modo Auto (`depos.js` `_autoOnBus` — escenas matching/scheduling/done + log "✅ cuenta ↔ tarjeta" / "❌ no jaló", buffer anti-carrera hasta `mission_id`) |
