...
| `activity` | `account_refreshed` | `_refresh_account_after_deposit()` en `deposits.py` (single/multi, tras el intento) **o** `account_refresh.run_refresh_cycle()` (bg-loop `_account_refresh_loop` en `app.py`, cada `ACCOUNT_REFRESH_INTERVAL_SEC`, cuentas con JWT vigente — sin login/captcha; `who=None`) | `{ts, email, target, balance_real, balance_total, who}` | repinta fila/detalle · modal v8 `onBusEvent` (balance fresco before→after, L2) |
...
| `activity` | `withdrawal_status` | `GET /api/accounts/{id}/withdraw/status/{tx_id}` en `app.py` (vía `withdrawals.resolve_withdrawal_status`) **Y** `account_refresh._withdrawal_resolution_loop` (bg-loop server-side cada 60s) — SOLO cuando el status pasa a terminal (`successful`/`completed`/`failed`) y no lo era ya (`new_terminal and not was_terminal`). El bg-loop resuelve retiros pendientes sin depender de ningún tab del navegador abierto (Bug 1 fix, 2026-08-07). | `{ts, target, id, transactionId, status, amount}` | **No va al feed** (`app.js`, mismo trato que `account_refreshed`) — dispara `_onAccountRefreshed({email: target})` (repinta tabla + detalle si está abierto) + `pushNotif` ✅/❌. Agregado 2026-07-26 junto con el fix de `withdraw_status` (ver `docs/ERRORS.md` §"Retiro se queda 'en proceso' para siempre"). El bg-loop server-side (2026-08-07) hace que la resolución no dependa del poll JS del navegador — antes, si nadie tenía el panel de esa cuenta abierto, el status nunca avanzaba. |
| `activity` | `withdrawal_ready_changed` | `account_refresh.run_refresh_cycle()` (bg-loop, mismo ciclo que `account_refreshed`) — verifica PASO1 de `withdrawals.get_bank_accounts()` reusando el JWT/proxy ya vivo del refresh de balance, SOLO emite cuando el valor cambia respecto al ciclo anterior (compara contra `accounts.withdrawal_ready` en BD, no dispara en cada ciclo) | `{ts, email, withdrawal_ready: bool, withdrawal_institution: str\|null}` | Cachea en `accounts.withdrawal_ready`/`withdrawal_institution` (migración `d798ab3`) — gatea el botón de retiro del portal sin round-trip vivo a BetMexico en cada render. `withdrawal_institution` puede ser `"Múltiples cuentas — revisar"` si BetMexico devuelve >1 cuenta aprobada (`MultipleApprovedAccounts`). |
...
| `activity` | `auto_mission` | `run_auto_mission()` en `auto_deposit.py` vía `_broadcast_mission()` — Modo Auto V2 (2026-07-28). Un evento por transición de fase | `{ts, mission_id, status, fake_pct, who, who_color, who_id, ...extra}`. `status` ∈ `started` (emitido por `POST /api/deposits/auto` al lanzar, con `accounts`), `matching` (fase 1, con `accounts`), `match` (par casado, con `email`/`card_tail`), `preparing` (piso 45-60s pre-Fase 2, mensaje genérico sin cifras — handoff 2026-08-05 Área B), `scheduling` (fase 2; progreso con `email`/`completed`/`total` o aborto con `aborted:code`), terminal ∈ `completed`/`failed`/`cancelled` (con `deposited`/`approved`/`failed`/`accounts` o `reason`; `stopped_by_user:true` si se declinó en el gate). `fake_pct` (int 0-100) es la **única fuente de verdad** de progreso, calculada por `auto_deposit._fake_progress_pct()` — consumida por bot y portal, sin fórmula duplicada en JS (handoff 2026-08-05 Área C). El endpoint cancel también emite `status:'cancelled'` directo desde `app.py` | `pushActivityEvent()` · app.js `connectSSE` (pushNotif en hitos terminales: 🤖 completada / ❌ falló / 🛑 detenida) · drawer Modo Auto (`depos.js` `_autoOnBus`) · `portal.js` `onMissionEvent` (consume `fake_pct` directo, `stopped_by_user` para ocultar deposited en cierres no-reales) |
