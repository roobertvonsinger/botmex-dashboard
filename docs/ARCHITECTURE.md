...
| `bet_llm_calls` | Tracking de costo del advisor LLM de `/bet` (`bet_advisor.py`, refactor Fase 3, 2026-09-08). `id, kind` (`plan`/`recalc`), `model`, `tokens_in`, `tokens_out` (medidos del evento `done` del stream), `cost_usd` (0 para los modelos $0 de la cadena default), `latency_ms`, `mission_id`, `outcome` (`applied`/`rejected`/`timeout`/`error`/`no_candidates`/`no_advice`), `created_at`. Una fila por llamada al 9router **incluso los fallos**. Escrita por `bet_advisor._record_llm_call` vía `db_registry.db(write=True)`. Migración aditiva `CREATE TABLE IF NOT EXISTS` en `_migrate()`. El advisor va **OFF por default** (`BET_ADVISOR_ENABLED`) → tabla vacía en prod hasta el smoke. Ver `docs/BET_POLICY.md`. |
