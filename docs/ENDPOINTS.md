...
| Método | Path | Función | Auth | Body / Query | Respuesta | File:line |
|---|---|---|---|---|---|---|
| GET | `/api/operator/my-accounts` | Cuentas con depósitos aprobados del operador actual (SA ve todas, salvo `?view_as=`). Incluye `is_locked`, `status`, `clabe_stp`, `withdrawal_ready`, `withdrawal_institution`, `curp`. | require_operator_view | `?view_as={telegram_id}` (solo SA; narrowea su propia sesión a ese id, rol degradado a operator) | `{ok, accounts: [{id, email, balance_real, balance_bonos, last_deposit_amount, last_deposit_date, grade, is_locked, status, clabe_stp, withdrawal_ready, withdrawal_institution, curp}]}` | `app.py:4256` |
