...
- **Flujo:** GO → valida pipes → **preview** inline (`.auto-preview`: "Voy a buscar las mejores cuentas para estas N tarjetas. ¿Dale?") → `POST /api/deposits/auto {card_pipes}` → `mission_id` → escenas vía bus propio (`onBusEvent` → `_autoOnBus`, con buffer anti-carrera hasta tener `mission_id`, patrón `_schedOnBus`). Si el inicio de la misión falla en el backend (ej. 409 Conflict por falta de cuentas), el `catch` del frontend invoca `showToast(msg)` para alertar al operador del motivo exacto antes de restablecer la UI.
...
**REDISEÑO 2026-07-28 (estado ACTUAL — reemplaza TODO lo de abajo, incl. el grid 2-filas de 2026-07-27):** rediseño completo en 4 rondas de campo, ver `DESIGN.md` §"Surface: La Pantalla" para el detalle de criterio de diseño. Resumen técnico:
- **Look**: laca grafito oscura pareja (ya NO rota de color por grade) + acento único `--pat-gold: var(--gold)` (brand real). Tipografía propia scoped (`--pat-font-display`/`--pat-font-mono` = IBM Plex Sans/Mono).
- **`.pat-columns` = CSS Grid de 3 columnas EXACTAMENTE iguales, una sola fila**: `grid-template-columns: minmax(0,1fr) minmax(0,1fr) minmax(0,1fr)`, `grid-template-areas: "ident stage txns"` — DATOS | DEPÓSITO/RETIRO | HISTORIAL. Historial (`.pat-col-txns`) dejó de ser una fila propia a todo el ancho debajo de las otras dos (así vivía desde 2026-07-27) — ahora es la 3ª columna; `minmax(0,1fr)`, no `1fr` a secas (un `1fr` simple deja que la columna con contenido menos encogible se robe espacio — medido). Fallback apilado `.pat-cramped` (scrollWidth>clientWidth, medido por JS, `_syncColumnsFit`) solo si ni así caben.
- **Panel único Depositar+Retirar, sin tabs**: un solo `#amtInput`, botones lado a lado (`.pat-dep-fire-row`), Reps/multi se esconden solos si no aplican. El contenido del panel tiene su propio `max-width:300px` (`.pat-dep-stage`) para no estirarse gigante aunque la columna sea ancha.
- **Alto de la ficha** (`pantalla.js` `_syncFichaHeight`): toma el MÁXIMO de las 3 columnas (ya no la suma de fila-arriba + fila-historial) — la ficha se acortó porque historial ya no pide su propia fila. `SOFT_CAP=480` sin cambios.
- El resto de esta sección (`.pat-col-ident`/`.pat-col-txns`/`.pat-col-stage` con `flex:`/`min-width:` de 2026-07-09, y el grid 2-filas de 2026-07-27) describe mecanismos VIEJOS — se mantienen abajo como historial de cómo se llegó aquí, no como referencia vigente.

**AUDITORÍA DE INTERACCIÓN 2026-08-01 (Fitts' Law + jerarquía — Robert, campo: "muy inoperable... los controles se sienten amontonados"):** auditoría hands-on (dev server aislado + medición real de `getBoundingClientRect`/`getComputedStyle`, no estimado) sobre `.pat-dep-stage` (Depositar/Retirar). Hallazgos con datos + fix:
- **Botones Depositar/Retirar (`.pat-act`, único uso en todo el repo) medían 26-30px de alto** — por debajo del piso Fitts' de escritorio (32px) pese a ser el control de MAYOR consecuencia del panel (dinero real, irreversible). Compartían tamaño con cualquier ghost button secundario. Sube a **40px/13px/700** (44px en el breakpoint mobile, ver abajo) — ahora el control más grande de la columna, por encima del saldo (26px, texto no accionable).
- **`#amtInput` medía 23px/11px** — el dato que el operador edita a mano más veces por sesión pesaba menos que una etiqueta. Ahora `.pat-dep-amt` es un campo bordeado propio (contenedor, no solo el `<input>`) a **40px/17px/700**, con `:focus-within` en `--pat-gold`.
- **`.amt-preset` (atajos de un click) medían 18px de alto** — más chico que un click rápido puede acertar bajo presión; un preset fallido cuesta MÁS que escribir el monto (el atajo que existía para ahorrar tiempo lo perdía). Sube a **min-height:30px/12px/600** (36px en mobile).
- **Retirar deshabilitado solo comunicaba el motivo vía `title` (hover)** — bajo presión no hay tiempo de pasar el mouse. `_applyWithdrawToCompact` (`pantalla.js`) ahora agrega el motivo (p.ej. "Saldo < $100") como texto SIEMPRE visible junto a `#wdBalance` (`.pat-wd-balance-note`), no solo en el tooltip.
- **BUG real cerrado — `.pat-col-stage { display:none }` en `@media (max-width:767px)`:** regla heredada de ANTES de que `.pat-columns` pasara a CSS Grid (2026-07-28); bajo ~768px (o cualquier ventana angosta con paneles laterales — el escenario real de Robert con "varios monitores/pestañas abiertas") **Depositar y Retirar desaparecían del DOM por completo**, no solo se veían mal. Violaba "no quitar, compactar" en la superficie que el ticket pedía arreglar. Fix: el breakpoint mobile ahora apila `.pat-columns` en grid (mismo mecanismo que `.pat-cramped`: `grid-template-areas: "ident" "stage" "txns"`) en vez de ocultar la columna; targets ≥44px extendidos a `#amtInput`/`.amt-preset` en ese breakpoint.
- **Criterio de jerarquización aplicado (reusable):** (1) el control que dispara la acción de mayor consecuencia recibe el mayor peso Fitts' de la superficie; (2) el campo que alimenta esa acción pesa un escalón abajo del control, pero arriba de cualquier label/nota; (3) un atajo de un click nunca va por debajo del piso de acierto (~28-30px) — si falla el click, ya no es atajo; (4) info de referencia (saldo, historial) nunca compite en tamaño con el control accionable que la usa; (5) el estado (deshabilitado/cargando/fallido) se comunica sin depender de hover; (6) ningún control operativo desaparece del DOM en ningún breakpoint, se compacta.
- Archivos: `static/pantalla.css` (`.pat-act`, `.pat-dep-amt`, `.pat-dep-stage .amt-preset`, `@media max-width:767px`), `static/pantalla.js` (`_applyWithdrawToCompact`). No se tocó `depos.js`/`depos.css` ni la POSICIÓN de `.pat-columns` (reasignación de orden ident/stage/txns es de otro agente en paralelo, fuera de alcance de esta auditoría).
...
2. **Copy "✅ Retiro liberado" → "✅ Retiro procesado — confirma en tu banco".** Contradecía la política ya establecida (bug#2 de `docs/ERRORS.md`: `status_api:6` significa que BetMexico ejecutó el retiro, NO que aterrizó en el banco). `pantalla.js` ya evita ese overclaim ("Confirma en tu banco"); el portal decía "liberado", que sí implica entrega. Alineado al mismo copy.
3. **Alertas de mismatch ausentes.** `fetchWithdrawStatus` ignoraba `st.alerts.{gatewayMismatch,digitsMismatch}` — señales anti-fraude que `pantalla.js` sí muestra (retiro salió a tarjeta en vez de SPEI, o a dígitos de cuenta distintos a los esperados). Ahora se disparan como toast adicional cuando vienen en la respuesta.
4. **`.mv-progress-fill` con `transition: width .4s ease` (portal.html) peleaba contra la interpolación JS.** `animateProgressTo` (rAF + `easeOutCubic`, 2200ms) escribe `style.width` en cada frame; con la transición CSS activa, cada escritura reinicia una transición de 400ms desde el valor actual (comportamiento de spec CSS Transitions) — el resultado real no seguía la curva de easing diseñada, producía micro-saltos. Se quitó la regla `transition: width` — JS ya es dueño exclusivo de la interpolación.

**Fix (mismo pase): CURP sentinel "N/A" se imprimía literal.** `acc.curp` llega como string `'N/A'` (no `null`) cuando BetMexico no lo tiene — `acc.curp ? ... : ''` es truthy para `'N/A'`, así que `renderAccountCard` mostraba "• CURP: N/A" al operador. Verificado en vivo contra copia de la DB real (34 cuentas). Ahora el check es `acc.curp && acc.curp !== 'N/A'`.
