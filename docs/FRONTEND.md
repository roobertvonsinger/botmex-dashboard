...
- **P3 — "Actualizar visibles" solo SA.** `#btnRefreshVisible` se oculta para no-SA en `loadMe()` (junto a los otros gates por rol). Operadores refrescan **individual** (`↻` por fila, `.row-refresh-one`) o por selección. El endpoint `/api/prewarm/refresh-stream` **NO se gatea por rol**: es compartido con el `↻` individual del operador (refrescar sus propias cuentas no es acción privilegiada); gatearlo rompería su flujo. El control es la visibilidad del botón (capa operador/SA), no el endpoint.
