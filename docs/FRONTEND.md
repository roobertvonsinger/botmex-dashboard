...
**Layout — REDISEÑO 2026-07-09 (`renderPantallaHead`, sin scroll de página, altura FIJA):** las secciones "Layout 2026-07-04/06" de abajo describen el diseño ANTERIOR (2 zonas: datos|transacciones-a-todo-ancho) — **ya no vigente**, se dejan como historial. El diseño actual:
- **`.pat-topbar`** (fila superior, full width): nombre·edad + grade a la izquierda, `.pat-actions` (Fijar/En uso/Depositar) a la derecha (`margin-left:auto`), `padding-right:50px` reserva el hueco de `.pantalla-close`.
- **`.pat-columns`** (fila, `flex:1 1 auto`) — **3 columnas reales**, cada una con su propio header fijo (no scrollea) + hairline dorado continuo de frontera:
  - **`.pat-col-ident`** (`flex:0 0 218px`): combo `email:password` (`.pat-combo-line`, ya no lleva `.pat-actions` — subieron a `.pat-topbar`) → saldo 36px → estado/nac/CURP → tarjetas/notas (`renderPantallaSaved`, capado 2+2, `margin-top:auto` las pega abajo).
  - **`.pat-col-txns`** (`flex:0 1 420px; min-width:260px`) — **compacta** (ya NO se estira a todo el ancho — antes el monto se iba al borde derecho con hueco muerto en medio). Header `.pat-txn-h` (ícono+"Movimientos"+contador) es HERMANO fijo de `.pat-txn-col` (antes vivía DENTRO del contenedor scrolleable y desaparecía al bajar la lista — bug encontrado y corregido de paso). `.pat-txn-col` es SOLO el área `overflow-y:auto` de filas.
  - **`.pat-col-stage`** (`flex:1 1 0`, `id="patStageSlot"`) — zona de animación del depósito (ver bloque "Escenario de depósito migrado" más abajo). Vacía en reposo.
...
> - Markup: `renderPantallaHead` en `pantalla.js` — el `<div class="pat-actions">` ahora es hijo de `.pat-combo-line`, no de `.pat-idrow`. Los botones conservan sus mismos `data-mark-email`/`data-inuse`/`data-acc-id`; la delegación de eventos de `#pantalla` no depende del padre, sin roturas.
