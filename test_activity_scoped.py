...
def test_recent_includes_own_interactions_and_marks(make_client):
    cli = make_client(role="user", telegram_id=555, username="lau")
    cli.post("/api/marks/toggle", json={"email": "b@test.com"})  # marca una AJENA
    data = cli.get("/api/recent").json()
    emails = {r["email"] for r in data["recent"]}
    assert "a@test.com" in emails        # depósito propio (cuenta visible)
    assert "c@test.com" in emails        # lock propio (cuenta visible)
    assert "b@test.com" not in emails    # AJENA: ley del pool — no se filtra al operador
